...
function updateWebDev($pdo, $username, $first_name, $last_name, 
	$date_of_birth, $country, $web_dev_id) {
...
function deleteWebDev($pdo, $web_dev_id) {
	$deleteWebDevProj = "DELETE FROM web_devs WHERE web_dev_id = ?";
...
function getAllInfoByWebDevID($web_dev_id) {
    global $pdo; // Assuming you're using a $pdo object for the database connection
    $stmt = $pdo->prepare('SELECT * FROM web_devs WHERE web_dev_id = :web_dev_id');
    $stmt->execute(['web_dev_id' => $web_dev_id]);
